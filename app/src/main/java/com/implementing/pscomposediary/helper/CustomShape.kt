package com.implementing.pscomposediary.helper
...
class CustomShape {
    object Shapes{
        ...
        val Pentagon = GenericShape { size, _ ->
            ...
            lineTo((startX + radius * cos(2 * PI / 5)).toFloat(),
                (startY + radius * sin(2 * PI / 5)).toFloat()
            )
            lineTo((startX + radius * cos(4 * PI / 5)).toFloat(),
                (startY + radius * sin(4 * PI / 5)).toFloat()
            )
            lineTo((startX + radius * cos(6 * PI / 5)).toFloat(),
